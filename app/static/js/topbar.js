    ...
    search_bar.addEventListener("keypress", function(event) {
        ...
        if (event.key === "Enter") {
            ...
            if (query.trim() === "") {
                ...
                window.location = "/user/search?q=" + encodeURIComponent(query);
            }
        }
    });
...
function displayRecommendations(music_recommendations, name_recommendations) {
    ...
    music_recommendations.forEach(item => {
        ...
        music_recommendation_item.href = "/user/music/" + item.id;
...
function search(){
    window.location.href="/user/search?q="+document.getElementById("search_bar").value;
}
